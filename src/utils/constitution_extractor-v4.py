...
class ConstitutionDocxExtractor:
    ...
    def __init__(self, docx_path):
        ...
        self.LIST_PARAGRAPH = 'List Paragraph'
        self.LIST_BULLET = 'List Bullet'
        self.LIST_NUMBER = 'List Number'
        self.NORMAL = 'Normal'
        
        # Track if we're inside a clause to better handle multi-line content
        self.in_clause_content = False
        self.current_clause_text = ""
        
        # Track if we're inside a sub-clause to better handle multi-line content
        self.in_sub_clause_content = False
        self.current_sub_clause_text = ""
        ...
        # Pattern for clauses with numbers in parentheses - more strict to avoid false positives
        self.clause_pattern = re.compile(r'^\(?\s*(\d+)\s*\)?\s+(.*)')
        # Pattern for sub-clauses with letters - more comprehensive
        self.sub_clause_pattern = re.compile(r'^\(?\s*([a-z])\s*\)?\s+(.*)')
        # Pattern for numbered sub-items (roman numerals)
        self.numbered_sub_clause_pattern = re.compile(r'^\(?\s*(i{1,3}|iv|v|vi|vii|viii|ix|x)\s*\)?\s+(.*)')
        # Pattern to detect if text is likely a clause (starts with number in parentheses)
        self.likely_clause_pattern = re.compile(r'^\(\d+\)')
        # Pattern to detect if text is likely a sub-clause (starts with letter in parentheses)
        self.likely_sub_clause_pattern = re.compile(r'^\([a-z]\)')
        # Pattern to detect if text is likely a continuation of previous text
        self.continuation_pattern = re.compile(r'^[a-z]')
    ...
    def to_title_case(self, text):
        ...
        # Don't title-case if it looks like a full sentence or clause content
        if len(text.split()) > 10 or text.startswith('('):
            return text
        ...
        if text.isupper():
            ...
            titled_words = []
            for word in words:
                if word in ['OF', 'THE', 'AND', 'IN', 'TO', 'FOR', 'ON', 'BY', 'AT', 'WITH']:
                    titled_words.append(word.lower())
                elif word in ['UN', 'EU', 'UK', 'US', 'ICC', 'WHO']:
                    titled_words.append(word)  # Keep abbreviations as is
                else:
                    titled_words.append(word.title())
            return ' '.join(titled_words)
        
        # If it's already in sentence case or mixed case, just ensure first letter is capitalized
    ...
    def extract_preamble(self, paragraphs):
        ...
        preamble_lines = []
        # Skip any empty paragraphs at the beginning
        start_idx = 0
        while start_idx < len(paragraphs) and not paragraphs[start_idx]['text']:
            start_idx += 1
            
        # Look for preamble content before the first chapter
        for i in range(start_idx, len(paragraphs)):
            ...
            # Skip if it looks like an article title (usually not in preamble)
            if self.article_title_pattern.match(text):
                continue
                
            # Add to preamble
            preamble_lines.append(text)
            
        # Join with proper spacing and newlines for readability
        if preamble_lines:
            return '\n'.join(preamble_lines)
        
        # Fallback preamble if none found
        return "We, the people of Kenya, acknowledging the supremacy of the Almighty God of all creation..."
    ...
    def process_paragraphs(self, paragraphs):
        ...
        while i < len(paragraphs):
            ...
            if chapter_match or style == self.HEADING1:
            ...
            elif self.article_title_pattern.match(text) or style == self.HEADING2:
                if current_chapter:
                    article_title = text.rstrip('.')
                    
                    # Check if this is actually clause content that should be in a clause, not a title
                    # Clause content is often longer and more sentence-like
                    if len(article_title.split()) > 12 and not article_title.startswith('Article'):
                        # This is likely clause content, not a title
                        if current_article:
                            # If we have no clauses yet, create one
                            if not current_article["clauses"]:
                                ...
                                logger.info(f"Created implicit clause for article content: {article_title[:30]}...")
                            else:
                                # Append to the last clause
                                current_article["clauses"][-1]["text"] += " " + article_title
                                logger.info(f"Appended to existing clause: {article_title[:30]}...")
                            i += 1
                            continue
            ...
            elif (style == self.LIST_PARAGRAPH or self.clause_pattern.match(text) or self.likely_clause_pattern.match(text)) and current_article:
                ...
                if clause_match:
                    ...
                    while j < len(paragraphs):
                        ...
                        if (self.sub_clause_pattern.search(next_text) or
                            ...
                            self.likely_sub_clause_pattern.match(next_text) or
                            next_style in [self.LIST_PARAGRAPH, self.LIST_BULLET, self.LIST_NUMBER]):
                            break
            ...
            elif current_clause and (style == self.LIST_PARAGRAPH or style == self.LIST_BULLET or 
                                    style == self.LIST_NUMBER or style.startswith('List') or 
                                    self.likely_sub_clause_pattern.match(text) or
                                    (style == self.NORMAL and current_article and 
                                     (current_article['title'].lower().startswith('national values') or 
                                      len(current_clause["sub_clauses"]) > 0 and self.continuation_pattern.match(text)))) and len(text) > 0:
