...
class ConstitutionDocxExtractor:
    ...
    def process_paragraphs(self, paragraphs):
        ...
        while i < len(paragraphs):
            ...
            # Articles without initial clause number (like Article 5, 8, 16, 18)
            elif self.article_number_pattern.match(text) and not current_article:
                article_match = self.article_number_pattern.match(text)
                if article_match and current_chapter:
                    article_number = article_match.group(1)
                    clause_number = article_match.group(2)
                    clause_text = re.sub(r'^(\d+)\.\s+\\?\((\d+)\\\)\s+', f'({clause_number}) ', text)
                    
                    # Save current article if exists
                    if current_article:
                        current_chapter["articles"].append(current_article)
                    
                    # Find the article title (should be previous paragraph)
                    article_title = ""
                    if i > 0:
                        prev_para = paragraphs[i-1]
                        title_match = self.article_title_pattern.match(prev_para['text'])
                        if title_match:
                    ...
                    logger.info(f"Found Article {article_number}: {article_title}")
            
            # Simple articles (like Article 8, 16, 18)
            elif not self.clause_pattern.match(text) and not self.sub_clause_pattern.match(text) and not self.numbered_sub_clause_pattern.match(text):
                # Check if this might be a simple article
                simple_article_match = re.match(r'^(\d+)\.\s+(.+)', text)
                if simple_article_match and current_chapter and not current_article:
                    article_number = simple_article_match.group(1)
                    article_text = simple_article_match.group(2)
                    ...
                    logger.info(f"Found Simple Article {article_number}: {article_title}")
...
def merge_fragmented_articles(self):
    ...
    def fix_article_numbers(self):
        ...
        # Known article titles and their correct numbers
        known_articles = {
            ...
            "National, economic and social policies": "10",
            "Culture": "11",
            ...
            "Application of Bill of Rights": "19",
            "Implementation of rights and fundamental freedoms": "20",
            "Enforcement of Bill of Rights": "21",
            "Authority of courts to uphold and enforce the Bill of Rights": "22",
            "Fundamental Rights and freedoms that may not be limited": "23",
            "Limitation of rights and fundamental freedoms": "24",
            "Fundamental duty of the State and every State organ": "25",
            "Right to life": "26",
        ...
        # Correct article numbers in each chapter
        for chapter in self.constitution["constitution"]["chapters"]:
            # First pass: correct known article titles
            for article in chapter["articles"]:
                if article["title"] in known_articles:
                    article["number"] = known_articles[article["title"]]
            
            # Second pass: ensure sequential numbering within each chapter
            article_numbers = sorted([int(a["number"]) for a in chapter["articles"] if a["number"].isdigit()])
            if article_numbers:
                expected_start = article_numbers[0]
                expected_end = expected_start + len(chapter["articles"]) - 1
                
                # If we have gaps or duplicates, renumber sequentially
                if len(set(article_numbers)) != len(article_numbers) or max(article_numbers) > expected_end:
                    logger.info(f"Renumbering articles in Chapter {chapter['number']} to be sequential")
                    # Sort articles by their current number if possible
                    chapter["articles"].sort(key=lambda a: int(a["number"]) if a["number"].isdigit() else float('inf'))
                    # Assign sequential numbers
                    for i, article in enumerate(chapter["articles"]):
                        article["number"] = str(expected_start + i)
    ...
    def clean_output(self):
        ...
        for chapter in self.constitution["constitution"]["chapters"]:
            ...
            chapter["title"] = re.sub(r'\s+[A-Z]$', '', chapter["title"])
            
            # If chapter title is empty, use the known title based on chapter number
            if not chapter["title"]:
                chapter["title"] = self.get_known_chapter_title(chapter["number"])
            ...
            for j, article in enumerate(chapter["articles"]):
                ...
                article["title"] = article["title"].rstrip('.')
                
                # Skip articles with no clauses or empty titles (likely fragments)
                if not article["clauses"] or not article["title"]:
                    articles_to_remove.append(j)
                    continue
                
                # Ensure article has a number
                if not article["number"] or not article["number"].isdigit():
                    # Try to infer from position in chapter
                    article["number"] = str(j + 1)
                    
                for clause in article["clauses"]:
                    # Clean clause text
                    clause["text"] = clause["text"].strip()
                    clause["text"] = re.sub(r'\\?\((\d+)\\\)', r'(\1)', clause["text"])
                    
                    # Fix Unicode characters
                    clause["text"] = clause["text"].replace('\ufb01', 'fi')
                    clause["text"] = clause["text"].replace('\ufb02', 'fl')
                    clause["text"] = clause["text"].replace('\u2014', '-')
                    clause["text"] = clause["text"].replace('\u201c', '"')
                    clause["text"] = clause["text"].replace('\u201d', '"')
                    
                    # Remove article title duplication in clause text
                    clause["text"] = re.sub(r'^\(\d+\)\s+[A-Za-z, ]+\.\s+\d+\.\s*', f'({clause["number"]}) ', clause["text"])
                    
                    for sub_clause in clause["sub_clauses"]:
                        # Clean sub-clause text
                        sub_clause["text"] = sub_clause["text"].strip()
                        
                        # Fix Unicode characters
                        sub_clause["text"] = sub_clause["text"].replace('\ufb01', 'fi')
                        sub_clause["text"] = sub_clause["text"].replace('\ufb02', 'fl')
                        sub_clause["text"] = sub_clause["text"].replace('\u2014', '-')
                        sub_clause["text"] = sub_clause["text"].replace('\u201c', '"')
                        sub_clause["text"] = sub_clause["text"].replace('\u201d', '"')
                        
                        # Remove duplicate semicolons
                        sub_clause["text"] = re.sub(r';\s*;', ';', sub_clause["text"])
                        sub_clause["text"] = re.sub(r';\s*and\s*;', '; and', sub_clause["text"])
                        
                        # Format sub-clause text to include letter
                        if not sub_clause["text"].startswith(f"({sub_clause['letter']})"): 
                            sub_clause["text"] = f"({sub_clause['letter']}) {sub_clause['text']}"
            
            # Remove articles marked for removal (in reverse order to avoid index issues)
            for j in sorted(articles_to_remove, reverse=True):
                if j < len(chapter["articles"]):
                    chapter["articles"].pop(j)
    ...
    def post_process_structure(self):
        ...
        # 2. Move sub-clause (a) Privacy to Article 31 (robust, recursive)
                ...
                for clause in art.get("clauses", []):
                    for sub in list(clause.get("sub_clauses", [])):
                        if (
                            ...
                            and art.get("number") != skip_article_number
                        ):
        ...
        if not found_31 and privacy_subs:
            last_ch = chapters[-1]
            new_art = {"number": "31", "title": "Privacy", "clauses": [{"number": "1", "text": "", "sub_clauses": privacy_subs}]}
            last_ch["articles"].append(new_art)

        # 2b. Add a placeholder for chapter preambles/notes
        for ch in chapters:
            if "preamble" not in ch:
                ch["preamble"] = ""  # For future use
        # 3. Fix Article 10 sub-values (e.g., 'sustainable development')
        for ch in chapters:
            for art in ch.get("articles", []):
                if art["number"] == "10":
        ...
        # 4. Expand sub-clauses for complex articles (e.g., Freedom of expression)
        # (This is a placeholder: real fix would need more context)
