...
# -*- coding: utf-8 -*-
"""
Constitution Extractor - Version 7
---------------------------------
...
This version focuses on extracting content starting directly from Chapter 1,
with improved article and clause detection.
"""
...
class ConstitutionDocxExtractor:
    ...
    def process_paragraphs(self, paragraphs):
        ...
        for text, style in paragraphs:
            ...
            # Enhanced article detection - look for patterns like "Right to life" or "Freedom of expression"
            is_article_title = (article_title_match or article_number_match or 
                               (style in ['Heading 3', 'Heading 4']) or
                               (len(text.split()) < 10 and 
                                (text.startswith('Right to') or 
                                 text.startswith('Freedom of') or 
                                 text.startswith('Protection') or
                                 'rights' in text.lower() or
                                 'freedom' in text.lower())))
            ...
            # Enhanced sub-clause detection - look for indented paragraphs
            if self.current_clause and text and text.startswith('   '):
                # This might be an implicit sub-clause
                if not self.current_clause["sub_clauses"]:
                    sub_clause = {
                        "letter": "a",
                        "text": text.strip()
                ...
                self.current_clause["sub_clauses"].append(sub_clause)
                logger.info(f"Found implicit sub-clause in Clause {self.current_clause['number']}")
                continue
